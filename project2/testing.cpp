...
Node* a_star(Node* initial_node){
    ...
    if (elapsed_seconds >= 5) {
        cout << "Solution took too long to find" << endl;
        return 0;  
    }
...
vector<vector<int>> createRandVector() {
    vector<vector<int>> vec(3, vector<int>(3)); // declare 3x3 vector
    srand(time(0)); // seed random number generator with current time

    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            int randNum = rand() % 9; // generate random number between 0 and 9
            bool isUnique = true; // flag to check if number is unique

            // check if number is already in the vector
            for (int k = 0; k < i; k++) {
                for (int l = 0; l < 3; l++) {
                    if (randNum == vec[k][l]) {
                        isUnique = false;
            ...
            // if number is not unique, generate a new random number
            if (!isUnique) {
                j--;
                continue;
            }

            // assign unique random number to current index
            vec[i][j] = randNum;
        }
    }
...
int main(){
    ...
    // Create the initial node and print
        vector<vector<int>> vec = createRandVector();
        Node *initial_node = create_initial_node(vec);
    ...
    float total_rate = (float) success/(fail+success);
    cout << "Succes/Totals: " << total_rate << endl;
